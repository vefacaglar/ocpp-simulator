...
async function onAdd() {
  const id = `CP-${String(store.chargePoints.length + 1).padStart(3, '0')}`
  await store.createChargePoint({ id, name: `Charge Point ${id}` })
}
...
<template>
  <div class="charge-point-list">
    <div class="panel-header">
      <h2>Charge Points</h2>
      <button class="btn-add" title="Add Charge Point" @click="onAdd">+</button>
    </div>
      ...
      <div v-else class="cp-items">
        ...
        >
          <div class="cp-info">
            ...
            <span class="cp-status" :class="cp.status">{{ cp.status }}</span>
          </div>
          <div class="cp-meta">
            <span>{{ cp.ocppVersion }}</span>
            <button class="btn-delete" title="Delete" @click="onDelete(cp.id, $event)">×</button>
          </div>
        </div>
      </div>
    </div>
...
.panel-header {
  ...
  padding: 0.75rem 1rem; border-bottom: 1px solid #2a2a3a; background: #16162a;
}
...
.cp-status { font-size: 0.7rem; padding: 1px 6px; border-radius: 3px; text-transform: uppercase; }
.cp-status.disconnected { background: #3a2020; color: #e06060; }
.cp-status.connecting { background: #3a3a20; color: #e0c060; }
.cp-status.connected { background: #203a20; color: #60e060; }
