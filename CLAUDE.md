...
- Target split is `simulator-api` for UI management/config only, `ocpp-gateway` as a dumb WebSocket edge, `message-processor` as CP-to-server response producer with stdout audit only, and `ocpp-core` for canonical raw logs, transactions, business decisions, and CSMS-initiated CALL publishing.
...
- Run Go tests per module directory; do not assume a single root `go test ./...` works. Target modules are `apps/simulator-api`, `apps/ocpp-gateway`, `apps/message-processor`, `apps/ocpp-core`, `apps/csms`, `packages/ocpp-protocol`, and `packages/ocpp-schemas`.
